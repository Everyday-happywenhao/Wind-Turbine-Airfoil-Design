...
class GAN(object):
		...
		def generator(self, c, z, reuse=tf.compat.v1.AUTO_REUSE, training=True):
				...
				with tf.variable_scope('Generator', reuse=reuse):
						...
						cpw = tf.layers.conv2d_transpose(cpw, int(depth_cpw / 2), kernel_size, strides=(2, 1),
														 padding='same')
						cpw = tf.layers.batch_normalization(cpw, momentum=0.9)  # , training=training)
						cpw = tf.nn.leaky_relu(cpw, alpha=0.2)
						#            cpw += tf.random_normal(shape=tf.shape(cpw), stddev=noise_std)

						cpw = tf.layers.conv2d_transpose(cpw, int(depth_cpw / 4), kernel_size, strides=(2, 1),
														 padding='same')
						cpw = tf.layers.batch_normalization(cpw, momentum=0.9)  # , training=training)
						cpw = tf.nn.leaky_relu(cpw, alpha=0.2)
						#            cpw += tf.random_normal(shape=tf.shape(cpw), stddev=noise_std)

						cpw = tf.layers.conv2d_transpose(cpw, int(depth_cpw / 8), kernel_size, strides=(2, 1),
														 padding='same')
						cpw = tf.layers.batch_normalization(cpw, momentum=0.9)  # , training=training)
						cpw = tf.nn.leaky_relu(cpw, alpha=0.2)
						#            cpw += tf.random_normal(shape=tf.shape(cpw), stddev=noise_std)
						...
						lbs = tf.add(tf.multiply(pw1, tf.log(lbs + EPSILON)), tf.multiply(pw2,
																						  tf.log(1 - lbs + EPSILON)))  # batch_size x n_data_points x n_control_points
						lc = tf.add(tf.lgamma(pw1 + 1), tf.lgamma(pw2 + 1))
						lc = tf.subtract(tf.lgamma(tf.cast(num_control_points, dtype=tf.float32)),
		...
		def discriminator(self, x, reuse=tf.compat.v1.AUTO_REUSE, training=True):
				...
				with tf.variable_scope('Discriminator', reuse=reuse):
						...
						q = tf.concat([q_mean, q_logstd], axis=1,
									  name='predicted_latent')  # batch_size x 2 x latent_dim
		...
		def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, directory='.'):
				...
				for t in range(train_steps):
						...
						# Generate fake data
						y_latent = np.random.uniform(low=self.bounds[0], high=self.bounds[1],
						...
						_, dlf, qdl = self.sess.run(
								[d_train_fake, d_loss_fake, q_loss],
						...
						y_latent = np.random.uniform(low=self.bounds[0], high=self.bounds[1],
													 size=(batch_size, self.latent_dim))
						noise = np.random.normal(scale=0.5, size=(batch_size, self.noise_dim))
						_, gl, rl, qgl = self.sess.run(
								[g_train, g_loss, r_loss, q_loss],
								feed_dict={self.c: y_latent, self.z: noise, q_target: y_latent}
						)

						# Append losses to lists
						dis_losses.append(dlr + dlf)
						gen_losses.append(gl + qgl)
		...
		def synthesize(self, latent, noise=None, return_cp=False):
				if isinstance(latent, int):
						...
						X, P, W = self.sess.run([self.x_fake_test, self.cp, self.w],
												feed_dict={self.c: latent, self.z: noise})
				else:
						N = latent.shape[0]
						if noise is None:
