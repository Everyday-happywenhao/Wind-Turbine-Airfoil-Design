...
mainFolderPath = 'E:\jwh_gan\MATLAB\verification\stall_aoa';
outputFolderPath = 'E:\jwh_gan\MATLAB\verification\judge_stalled_-AoA';
...
for i = 1:length(csvFiles)
    ...
    % 从文件名中提取雷诺数
    ReMatch = regexp(csvFileName, 'Re_(\d+)', 'tokens');
    if isempty(ReMatch)
    ...
    actualAOAIndex = find(angleData.Re_num == ReNum & angleData.AoA < 0, 1);
    if isempty(actualAOAIndex)
        warning('没有找到对应雷诺数负实际攻角：Re_%s', ReNumStr);
        continue;
    end
    ...
    % 输出文件路径
    outputFileName = sprintf('Re_%s.csv', ReNumStr);
