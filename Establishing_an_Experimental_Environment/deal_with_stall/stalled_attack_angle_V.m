% 主文件夹路径和输出文件夹路径
mainFolderPath = 'E:\jwh_gan\MATLAB\verification\CL_CD\every_condition_new';
outputFolderPath = 'E:\jwh_gan\MATLAB\verification\stall_aoa';
...
for i = 1:length(subFolders)
    ...
    % 从子文件夹名称提取雷诺数信息
    ReMatch = regexp(subFolders(i).name, 'Re_(\d+)', 'tokens');  % 假设子文件夹名称含有 "Re_"
    if ~isempty(ReMatch)
